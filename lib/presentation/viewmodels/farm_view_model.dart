  ...
  Future<void> loadForCurrentUser() async {
    ...
    try {
      ...
      if (user.role == 'manager' && user.farmId != null) {
        dev.log('👷 Manager role detected → fetching farm ${user.farmId}',
            name: 'FarmViewModel');
        _singleFarm = await _repository.getById(user.farmId!);
        _farmList = [_singleFarm!];
        _currentFarm.set(_singleFarm);
        dev.log('✅  Fetched single farm',
            name: 'FarmViewModel',
         );
      } else {
        dev.log('👤 Owner role detected → fetching farms by user ${user.uid}',
            name: 'FarmViewModel');
        ...
        dev.log('✅  Fetched ${_farmList.length} farms',
            name: 'FarmViewModel');
      }
      ...
      _error = e is Exception ? e : Exception(e.toString());
      dev.log('❌  Error in loadForCurrentUser',
          name: 'FarmViewModel',
          level: 1000, // severe
          error: _error,
          stackTrace: stack);
      ...
      _isLoading = false;
      notifyListeners();
      dev.log('⏹  loadForCurrentUser() completed',
          name: 'FarmViewModel', level: 800);
    }
  }
